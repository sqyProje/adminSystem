...
<script type="text/ecmascript-6">
  ...
  import {InitOldFinishList} from '@/api/approve'
  ...
  export default {
    ...
    methods: {
      ...
      initTable() {
        InitOldFinishList(this.listQuery).then(response => {
      ...
      handleSeek(row) {
        this.$router.push({name:'oldsee',query: {u_id: row.approveid,pageNum:this.listQuery.pageNum, formName:'old_finishApprove'}})
      },
      HandleWorkFlow(row){
        this.$router.push({name:'oldworkflow',query: {u_id: row.approveid,pageNum:this.listQuery.pageNum, formName:'old_finishApprove'}})
      },
      HandlePrint(row){
        this.$router.push({name:'oldprint',query: {u_id: row.approveid,pageNum:this.listQuery.pageNum, formName:'old_finishApprove'}})
      }
    }
  }
